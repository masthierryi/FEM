...
%% FEM setup ==============================================================
    function self  = FEM_setup(self)
        ...
        % self.mesh.adim_coordinates = linspace(0,1,self.mesh.n_el);
    end
...
%% geometry ===============================================================
    function self = parameters(self,BT)
        ...
        % material --------------------------------------------------------
                                ...
                                % self.Beam(1:no,7) = self.data.E/(2*(1 + ...
                                %                        self.Beam(1:no,5)));
                                self.Beam(1:no,7) = 80.8e9;
        ...
        elseif BT == 4 % Timoshenko BT
            ...
            s_t = sqrt((self.Beam(1:no,4)./(self.Beam(1:no,3).* ...
                self.Beam(1:no,7))).*r_t.^2);
                                                    ...
                                                    self.data.s_t = s_t(1);
                                                            self.S = 1/r_t;
                self.gama = sqrt(self.Beam(4)/(self.Beam(3)*self.Beam(7)));
        ...
        % 1- A; 2- I; 3- k; 4- E; 5- rho; 6- nu; 7- G; 8- r; 9- s # self.Beam()
...
%% mesh ===================================================================
    function self = Eigenproblem(self)
        ...
        for i = 1:self.mesh.n_el % Loop for each element
            ...
            % % Location in the global matrix -----------------------------
            % Loc = [self.ID(1, self.mesh.incidence(i, 2)),...
            %     self.ID(2, self.mesh.incidence(i, 2)),...
            %     self.ID(1, self.mesh.incidence(i, 3)),...
            %     self.ID(2, self.mesh.incidence(i, 3))];
            % 
            ...
            %         for k = 1:4 %Roll
            %             if Loc(k) ~= 0
            %                 self.GK(Loc(j), Loc(k)) = self.GK(Loc(j), Loc(k)) + M_k(j, k);
            %                 self.GM(Loc(j), Loc(k)) = self.GM(Loc(j), Loc(k)) + M_m(j, k);
            %             end
            %         end
            %     end
            % end
            ...
            % for n=1:self.mesh.n_el
                for k=((2*i)-1):(2*(i+1)) 
                    for j=((2*i)-1):(2*(i+1))
                        self.GM(k,j) = self.GM(k,j) + M_m((k-(2*(i-1))),(j-(2*(i-1))));
                        self.GK(k,j) = self.GK(k,j) + M_k((k-(2*(i-1))),(j-(2*(i-1))));
                    end
                end
            % end 
...
%% Solving the eigen problem ==============================================
    function self = EigenSolve(self,BT)
        ...
        [self.natfreq,Ind] = sort(omega);
    
        % Natural frequencie (hz)
        self.natfreqHz = self.natfreq./(2*pi);

        % Removing rigid body movement
        self.data.RBM = length(self.natfreq(self.natfreq<4));
        ...
        elseif BT == 2 % Rayleight Beam -----------------------------------
            ...
            self.freq(:,1) = B*(sqrt(self.eigenvalues(:,1).^2 - self.eigenvalues(:,2).^2)/2*pi*r);
        ...
        elseif BT == 3 % Shear Beam ---------------------------------------
            ...
            self.freq(:,1) = B*(sqrt(self.eigenvalues(:,1).^2 - self.eigenvalues(:,2).^2)/2*pi*s);
        ...
        elseif BT == 4 % Timoshenko Beam ----------------------------------
            ...
            for j = 1:size(b)
                if b(j) < self.critical(3)
                    ...
                    self.freq(j,1) = B*(sqrt(self.eigenvalues(j,1).^2 - self.eigenvalues(j,2).^2)/ ...
                        (2*pi*sqrt(r.^2+s.^2)));
                elseif b(j) > self.critical(3)
                    % Natural frequency after critical frequency
                    self.freq(j,2) = B*(sqrt(self.eigenvalues(j,1).^2 + self.eigenvalues(j,3).^2)/ ...
                        (2*pi*sqrt(r.^2+s.^2)));
                end 
            end
        ...
        if BT == 1 %Euler-Bernoulli -----------------------------------
            self.eigenvalues = sqrt(self.natfreq.*self.data.L.*sqrt(self.Beam(5)/...
                self.Beam(4))*self.S(1));

        elseif BT == 2 %Rayleight Beam --------------------------------
            B = sqrt(self.Beam(5)*self.Beam(1)/(self.Beam(4)*self.Beam(2)))*...
                self.natfreq*self.data.L^2;

            % Alpha
            self.eigenvalues(:,1) = (B./sqrt(2)).*sqrt(-(1/self.S(1))^2 + sqrt((1/self.S(1))^4 + 4./B.^2));
            % Beta
            self.eigenvalues(:,2) = (B./sqrt(2)).*sqrt((1/self.S(1))^2 + sqrt((1/self.S(1))^4 + 4./B.^2));
        ...
        elseif BT == 3 %Shear Beam ------------------------------------
            ...
            % Alpha
            self.eigenvalues(:,1) = (B./sqrt(2)).*sqrt(-(1/self.S(1)*self.gama(1))^2+...
                sqrt((1/self.S(1)*self.gama(1))^4 + 4./B.^2));
            % Beta
            self.eigenvalues(:,2) = (B./sqrt(2)).*sqrt((1/self.S(1)*self.gama(1))^2+...
                sqrt((1/self.S(1)*self.gama(1))^4 + 4./B.^2));
        ...
        elseif BT == 4 %Timoshenko Beam -------------------------------
            ...
            self.eigenvalues(:,3) = (B./sqrt(2)).*sqrt(((1/self.S(1))^2 +...
                ((1/self.S(1))*self.gama(1))^2) - sqrt( ((1/self.S(1))^2 -...
                ((1/self.S(1))*self.gama(1))^2)^2 + 4./B.^2));

            % Alpha after the critical frequencie
            self.eigenvalues(:,2) = (B./sqrt(2)).*sqrt(-((1/self.S(1))^2 +...
                ((1/self.S(1))*self.gama(1))^2) + sqrt( ((1/self.S(1))^2 -...
                ((1/self.S(1))*self.gama(1))^2)^2 + 4./B.^2));

            % Beta
            self.eigenvalues(:,1) = (B./sqrt(2)).*sqrt(((1/self.S(1))^2 +...
                ((1/self.S(1))*self.gama(1))^ 2) + sqrt( ((1/self.S(1))^2 -...
                ((1/self.S(1))*self.gama(1))^2)^2 + 4./B.^2));
        ...
        cont_disp = sum(self.data.BC(:,3));
        cont_rot = sum(self.data.BC(:,2));
        ...
        for i=1:length(disp)
            self.U_disp(i-1+self.mesh.BC_v(1),:) = A_vet(disp(i),:);
        end
        ...
        for i=1:length(rot)
            self.U_rot(i+1+self.mesh.BC_v(2),:) = A_vet(rot(i),:);
        end
...
%% Ploting the mode shapes 
        function self = ShapeModes(self,BT)
            ...
            % Data display
            frequencies(1,:) = self.eigenvalues(1+RBM:modes+RBM);
            display(frequencies)
            ...
            for  mode_cnt = 1:modes
                plot(self.mesh.coordinates/self.data.L,  ...
...
%% All theories shape modes comparing
        function self = AllTheories(self)
